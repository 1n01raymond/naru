...
`extras.naru.progressive.reducedChunks` under `naru.progressive-package.2`,
each carrying the measured `maxDeviationMeters` of its own prototypes so the
runtime decides a level per chunk rather than per package;
...
compares substituted frames against a `target`-only reference.
