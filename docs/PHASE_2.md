...
| Shape-preserving LOD | **Implemented and recorded** ([ADR-0025](adr/0025-shape-preserving-lod-representation.md), Accepted): `--reduced-lod` emits a `reduced` level under `naru.progressive-package.1` ([gate 1](../artifacts/lod/reduced-level/README.md)), and the Studio substitutes it while the document's declared deviation projects to 1.0 px or less, returns to `target` past 1.5 px, and pins the selected object to `target`; the [headed record](../artifacts/lod/reduced-selection/README.md) shows 99.23% and 99.38% of drawn geometry pixels within 8/255 of a `target`-only reference and identical picked ids on a 111-point lattice per engine, in Chrome and Firefox | Off by default and recorded on a four-part STEP corpus only; a real-model package with declared per-prototype bounds is the next question, not a screen-error policy change |
...
4. One shape-preserving LOD vertical slice with an explicit error and identity
   ...
   emits a second geometry level whose deviation is measured and declared, the
   Studio substitutes it only while that deviation projects to 1.0 px or less
   (returning to `target` past 1.5 px, and never for the selected object), and
   the [headed record](../artifacts/lod/reduced-selection/README.md) compares a
   substituted frame against a `target`-only reference in Chrome and Firefox.
   It is off by default and recorded on a four-part STEP corpus only. What
   remains is a package that declares per-prototype bounds, which is what makes
   the decision per prototype rather than one boolean per frame.
...
backlog; shape-preserving LOD is implemented and recorded on a STEP corpus, and
what it still owes is a real-model package with declared per-prototype bounds. Workspace persistence
...
| Reduced LOD level | [#76](https://github.com/1n01raymond/naru/issues/76), ADR-0025 gate 1 | **Recorded** in [`artifacts/lod/reduced-level/`](../artifacts/lod/reduced-level/README.md) (`pnpm lod:reduced:check`): two fresh compiles of the corpus with `--reduced-lod 0.001` are byte-identical under `naru.progressive-package.1`; `reduced` is emitted for `thin-plate-holes` (1,712 -> 310 triangles) and `fillet-bracket` (784 -> 162), withheld for `curved-shell` (sampled p95 above the bound) and `planar-control` (no reduction); 0 Khronos errors. Digests are host-local. |
| Reduced LOD selection | [#76](https://github.com/1n01raymond/naru/issues/76), ADR-0025 gate 3 | **Recorded** in [`artifacts/lod/reduced-selection/`](../artifacts/lod/reduced-selection/README.md) (`pnpm lod:selection:check`): at two camera distances on the same package, a frame drawn with the reduced level substituted agrees with a `target`-only reference on 99.9943% and 99.9992% of viewport pixels within 8/255 per channel — 99.2318% (35 of 4,556) and 99.3773% (5 of 803) of the pixels the geometry actually covers — while triangles fall 6,159 -> 4,135 and 111 lattice picks per engine return identical object ids. Recorded in Chrome and Firefox; the engines differ in frame coverage (614,259 against 613,738 pixels) and place the geometry one pixel apart in X, and the record carries that rather than asserting it away. One four-part STEP corpus, one declared bound; nothing about a real model follows from it. |
