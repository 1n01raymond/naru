...
| Runtime memory ownership | [#121](https://github.com/1n01raymond/naru/issues/121), [#125](https://github.com/1n01raymond/naru/issues/125) | #125's access half is measured: on the sixty5 relocated sidecar (188,319 entries, 110,146 relocated) the decoder retained 74.10 MiB in 179.9 ms and now retains 46.36 MiB in 97.2 ms (`node --expose-gc`, forced collection, medians of three same-session runs), because per-node transform copies cost 20.17 MiB that no production reader touches and `prototypeId` repeats 34 distinct values across every row. Package bytes, the sidecar format, and all 17 counters of [`artifacts/ifc/relocated-hierarchy-browser/`](../artifacts/ifc/relocated-hierarchy-browser/README.md) are unchanged. What remains is #121's document/Worker retention; no format question is settled by this. |
