...
The 4.743 s browser result above starts from an existing compiled package. It
must not be reported as a 4.743 s first source import. The complete product
...
| Cold/warm startup, frame, memory, and interaction results published | The [results matrix](REAL_LARGE_RESULTS.md) brings cache distributions, first frames, localized demand, and memory together, and Chrome and Firefox startup and memory records cover the same disclosed Windows host. | Every figure is Windows x64 on one discrete-GPU host, so no cross-platform claim follows from it; the second-operating-system repeat is non-blocking evidence debt under the scope decision below. The matrix spans distinct compilations and is not one end-to-end timeline. | **Met** |
| Forced low-memory scenario remains functional | The [Chrome memory envelope](../artifacts/memory/sixty5-envelope/README.md) and [Gecko repeat](../artifacts/memory/sixty5-envelope-gecko/README.md) exercise default and forced-low 8 MiB profiles with identical settled resident sets, navigation, selection, and eviction. | Both engines were driven from the same Windows host; that repeat is non-blocking evidence debt under the scope decision below. Gecko misses the predeclared 4 GiB process working-set ceiling and GPU driver allocation is unavailable on every engine measured; both are reported in the records rather than widened away. | **Met** |
...
**Scope decision, 2026-09-12.** The project owner scoped the second and third
criteria to results published for one disclosed host across two browser
engines. A repeat on a second operating system remains worth doing and remains
listed as evidence debt below; it no longer holds either criterion open. No
record changed and no claim widened: every startup and memory figure this
...
| Per-document IFC extraction reuse | **Recorded** under proposed [ADR-0019](adr/0019-document-artifact-transport.md) slices 1, 2, and 3a: artifacts are `naru.ifc-document-artifact.4`, a canonical-JSON structure region followed by the bulk numeric geometry and the per-document property columns as raw little-endian regions, verified by header checks and one SHA-256 over the stored payload bytes before any parse ([tests](../native/adapter-ifc/tests/test_document_artifact_cache.py), [clean adapter-merge equivalence](../native/adapter-ifc/tests/test_document_artifact_integration.py)); the [rebuild record](../artifacts/cache/rebuild-stages/README.md) measures a changed-discipline rebuild against a same-session clean rebuild on Digital Hub (10,302.4 vs 53,754.4 ms whole process, peak working set 0.44 vs 1.85 GB, unchanged-document verification 25.6 ms and parse 136.7 ms) and sixty5 (61,672.4 vs 316,082.6 ms, 2.96 vs 4.63 GB, verification 229.5 ms and parse 3,664.1 ms), byte-identical packages in both arms and the slice 1 package digests reproduced unchanged | In-compiler federation assembly (slice 3b) |
...
| Shape-preserving LOD | **Implemented and recorded** ([ADR-0025](adr/0025-shape-preserving-lod-representation.md), Accepted): `--reduced-lod` emits a `reduced` level under `naru.progressive-package.2` ([gate 1](../artifacts/lod/reduced-level/README.md)), every reduced chunk declares its own measured bound, and the Studio decides per chunk: a chunk is substituted while its own deviation projects to 1.0 px or less, returns to `target` past 1.5 px, and the selected object is pinned to `target`; the [headed record](../artifacts/lod/reduced-selection/README.md) shows 99.01% to 99.23% of drawn geometry pixels within 8/255 of a `target`-only reference, identical picked ids per engine, and one camera distance where the two bounds straddle the threshold so one prototype is drawn reduced while the other stays exact, in Chrome and Firefox | Off by default and recorded on a four-part STEP corpus only; whether a real model's bounds spread far enough to matter is the next question, not a screen-error policy change |
| Persistent browser cache and cache-aware eviction | **Implemented**, on by default in the Studio at a 256 MiB quota (`?persistentCacheMiB=`, `0` disables): the ADR-0011 transport resolves a digest-declared resource from a content-addressed Cache Storage tier before the network, re-hashes every hit against the same declared identity, refuses mismatching bytes, protects the open scene's resources from LRU eviction, and reports encoded persistent bytes apart from decoded/GPU residency ([ADR-0024](adr/0024-persistent-package-cache.md), [fake-storage tests](../packages/runtime-webgpu/test/package-cache.test.ts)); only sidecars declare digests today, so the document, `scene.bin`, and `coarse.bin` stay network-only | Declared digests for whole buffers at the `naru.` schema bump (ADR-0024 gate 1), then one focused unchanged-reopen record (gate 2) |
...
| Shape-preserving LOD | Error/identity contract (**Accepted**, [ADR-0025](adr/0025-shape-preserving-lod-representation.md)) → compiler representation (**Implemented**, opt-in) → scheduler admission and projected-error selection (**Implemented**, opt-in) → visual and interaction evidence (**Recorded** on the STEP corpus, Chrome and Firefox) | An ordering heuristic is not an LOD representation and cannot establish fidelity |
| Persistent browser cache | Package integrity verification and resource limits (**Implemented**) → quota/version policy (**Implemented**, [ADR-0024](adr/0024-persistent-package-cache.md), sidecars only until whole buffers declare digests) → cache-aware scheduler | Persisting unbounded or integrity-unverified remote resources enlarges the trust boundary |
...
1. Land the successor to the rejected payload tier that
   ...
   same-session clean arm. Slice 2, the column-form structure
   (`naru.ifc-document-artifact.3`: canonical-JSON structure records plus the
   bulk numeric geometry as raw little-endian regions), landed 2026-09-13, and
   slice 3a moved property interning into each document and stored its value
   heap and offset columns the same raw way
   (`naru.ifc-document-artifact.4`); the record was re-recorded after each.
   All four gates are met on both models after every slice; at slice 3a that
   is Digital Hub 10,302.4 against 53,754.4 ms and sixty5 61,672.4 against
   316,082.6 ms with peak memory lower in both, unchanged-document
   verification down from 4,434.2 to 25.6 ms and from 34,204.2 to 229.5 ms
   since gate 0, and both slice 1 package digests reproduced unchanged. One
   slice remains: in-compiler federation assembly (slice 3b), which owns the
   compiler's own structure scan -- `readSceneIr` is flat across both arms in
   the record (1.9 s on Digital Hub, 25.2 s on sixty5) because it reads the
   merged federation the adapter still writes. The ADR's gate 4 is the same
   predeclared bar after every slice: a
...
4. One shape-preserving LOD vertical slice with an explicit error and identity
   contract has landed under accepted
   [ADR-0025](adr/0025-shape-preserving-lod-representation.md): `--reduced-lod`
   emits a second geometry level whose deviation is measured and declared per
   prototype, and the Studio decides per chunk: it substitutes a chunk only
   while that chunk's own deviation projects to 1.0 px or less (returning to
   `target` past 1.5 px, and never for the selected object). The
   [headed record](../artifacts/lod/reduced-selection/README.md) compares
   substituted frames against a `target`-only reference at three camera
   distances in Chrome and Firefox, one of which puts the camera inside the
   band where the corpus's two bounds straddle the threshold, so one prototype
   is drawn reduced while the other stays exact in the same frame. It is off by
   default and recorded on a four-part STEP corpus only. What remains is a real
   model, where whether bounds spread far enough for the per-chunk decision to
   change a frame is an open question.
...
8. Make relocation the compiler default. Both halves of
   ...
   browser record puts that at -15.65% on first frame and -18.56% on peak heap
...
a missing capability. Shared-cache authorization remains implementation
backlog; shape-preserving LOD is implemented and recorded on a STEP corpus with
per-prototype bounds, and what it still owes is a real-model package. Workspace persistence
...
| Cross-platform real-large startup | Repeat the [first-frame record](../artifacts/ifc/sixty5-first-frame/README.md) on a second operating system with the same package and viewport; the [Gecko repeat](../artifacts/ifc/sixty5-first-frame-gecko/README.md) closes the second-engine half on this Windows host only, and both engines were driven from the same Windows machine | A published startup claim that is not specific to one Windows host. It no longer blocks the second exit criterion, which the 2026-09-12 scope decision narrowed to one disclosed host |
| Cross-platform memory envelope | Repeat the [phase-sampled memory ledger](../artifacts/memory/sixty5-envelope/README.md) on a second operating system; the [Gecko repeat](../artifacts/memory/sixty5-envelope-gecko/README.md) closes the second-engine half on this Windows host only, carries no heap figure because that engine exposes neither estimator, and does not meet the predeclared 4 GiB process working-set ceiling | A memory claim that is not specific to one Windows host. It no longer blocks the third exit criterion, which the 2026-09-12 scope decision narrowed to one disclosed host |
| Persistent package cache in a browser | One focused headed record of an unchanged Digital Hub reopen showing every digest-declared sidecar served from the tier with no network read for it and identical decoded/GPU totals ([ADR-0024](adr/0024-persistent-package-cache.md) gate 2); the tier is proved at unit scale only | Any claim that a reopen is faster or lighter because of the persistent tier |
...
| Runtime memory ownership | [#121](https://github.com/1n01raymond/naru/issues/121), [#125](https://github.com/1n01raymond/naru/issues/125) | #125's access half is measured: on the sixty5 relocated sidecar (188,319 entries, 110,146 relocated) the decoder retained 74.10 MiB in 179.9 ms and now retains 46.36 MiB in 97.2 ms (`node --expose-gc`, forced collection, medians of three same-session runs), because per-node transform copies cost 20.17 MiB that no production reader touches and `prototypeId` repeats 34 distinct values across every row. Package bytes, the sidecar format, and all 17 counters of [`artifacts/ifc/relocated-hierarchy-browser/`](../artifacts/ifc/relocated-hierarchy-browser/README.md) are unchanged. #121's first half is now measured on the same compiled bytes: preparation is the last reader of the document's `nodes` and `scenes`, so the prepared decoder keeps a shallow copy without them and a caller that drops its own reference releases the node graph for the session. Retained heap falls 99.47 MiB of 455.96 (sixty5, 188,320 nodes), 41.00 of 337.65 (relocated), and 6.84 of 37.49 (Digital Hub) over three paired runs per package under `node --expose-gc` with forced collection, decoded summary, evidence, and bounds identical in both arms. Those are retained-heap figures for one process, not a whole-application bound. What remains is a browser-side repeat and the Worker-side sidecar copies; no format question is settled by this. The browser half's measurement contract is predeclared in [`artifacts/memory/document-retention/`](../artifacts/memory/document-retention/README.md): pinned package and options, the two candidates admitted from the runtime ownership map in [`docs/RUNTIME.md`](RUNTIME.md), separate memory and timing run sets, sampling phases, failed-run handling, and the threshold below which nothing is landed. **The browser half is now measured against it** in [`artifacts/memory/document-retention/`](../artifacts/memory/document-retention/README.md) (`pnpm memory:retention:check`): the first of the two admitted candidates, the redundant main-thread document parse, was implemented and measured over 24 accepted runs with no discards, three memory pairs and three timing pairs per package. Peak main-thread used JS heap falls from 2,122,989,603 to 962,369,086 B (−54.67%) on the pinned sixty5 package and from 1,745,846,014 to 865,013,123 B (−50.45%) on its relocated variant, against a declared −10% threshold; the first coarse frame improves 20.9% and 17.5% against a +5% tolerance; and every accepted run preserved the assembly tree, source-aware picking, property resolution, selected-object detail, and the pinned resident endpoint. Two costs are recorded rather than hidden: the assembly tree arrives 511 ms (pinned) and 167 ms (relocated) later because it now waits for the Worker's initialization response, a milestone no declared guard covers, and the whole-cluster `measureUserAgentSpecificMemory()` figure barely moves because the parse changed threads rather than disappearing. The second candidate, the scene-replacement overlap, is untouched. |
...
| First LOD experiment | [#126](https://github.com/1n01raymond/naru/issues/126), supporting #76 | **Recorded** in [`artifacts/lod/method-comparison/`](../artifacts/lod/method-comparison/README.md) (`pnpm lod:method:check`): on a four-part STEP corpus with predeclared fidelity, edge, section, silhouette, and identity checks, OCCT retessellation at a declared tolerance (no coarser than 0.15 mm / 0.15 rad) is the only method that passes on the curved part; unconstrained meshoptimizer 1.2.0 passes with 0 identity conflicts only on planar-dominant parts whose CAD boundary edges travel as explicit edge segments; locked-boundary simplification cannot reduce those parts at all. 32 of 40 arm-part cells pass. The record settles method and topology class for #76, not a screen-error policy, and does not generalize to sixty5. |
| Reduced LOD level | [#76](https://github.com/1n01raymond/naru/issues/76), ADR-0025 gate 1 | **Recorded** in [`artifacts/lod/reduced-level/`](../artifacts/lod/reduced-level/README.md) (`pnpm lod:reduced:check`): two fresh compiles of the corpus with `--reduced-lod 0.001` are byte-identical under `naru.progressive-package.2`; `reduced` is emitted for `thin-plate-holes` (1,712 -> 310 triangles) and `fillet-bracket` (784 -> 162), withheld for `curved-shell` (sampled p95 above the bound) and `planar-control` (no reduction); 0 Khronos errors. Digests are host-local. |
| Reduced LOD selection | [#76](https://github.com/1n01raymond/naru/issues/76), ADR-0025 gate 3 | **Recorded** in [`artifacts/lod/reduced-selection/`](../artifacts/lod/reduced-selection/README.md) (`pnpm lod:selection:check`): at three camera distances on the same package, frames drawn with the reduced level substituted agree with a `target`-only reference on 99.0060% to 99.2336% of the pixels the geometry actually covers, within 8/255 per channel, while triangles fall 6,159 -> 4,135. At the nearest distance the two declared bounds straddle the 1.0 px threshold, so `fillet-bracket` (0.7274 px) is drawn reduced while `thin-plate-holes` (1.043 px) stays exact — a frame a package-wide bound could not produce — and the validator asserts that mixed case rather than taking the agreement ratio on trust. Picked ids are identical per engine at every distance. Recorded in Chrome and Firefox; the engines differ in frame coverage (614,259 against 613,738 pixels) and place the geometry one pixel apart in X, and the record carries that rather than asserting it away. One four-part STEP corpus; nothing about a real model follows from it. |
