...
| Per-document IFC extraction reuse | **Recorded** under proposed [ADR-0019](adr/0019-document-artifact-transport.md) slices 1 and 2: artifacts are `naru.ifc-document-artifact.3`, a canonical-JSON structure region followed by the bulk numeric geometry as raw little-endian regions, verified by header checks and one SHA-256 over the stored payload bytes before any parse ([tests](../native/adapter-ifc/tests/test_document_artifact_cache.py), [clean adapter-merge equivalence](../native/adapter-ifc/tests/test_document_artifact_integration.py)); the [rebuild record](../artifacts/cache/rebuild-stages/README.md) measures a changed-discipline rebuild against a same-session clean rebuild on Digital Hub (10,631.0 vs 52,570.5 ms whole process, peak working set 0.48 vs 1.83 GB, unchanged-document verification 28.7 ms and parse 167.3 ms) and sixty5 (70,667.5 vs 317,588.2 ms, 3.61 vs 5.11 GB, verification 329.0 ms and parse 3,161.0 ms), byte-identical packages in both arms and the slice 1 package digests reproduced unchanged | In-compiler federation assembly (slice 3) |
...
1. Land the successor to the rejected payload tier that
   ...
   the record was re-recorded again. All four gates are met on both models
   after each slice; at slice 2 that is Digital Hub 10,631.0 against
   52,570.5 ms and sixty5 70,667.5 against 317,588.2 ms with peak memory
   lower in both, artifact parse down from 650.6 to 167.3 ms and from
   5,482.0 to 3,161.0 ms, and both slice 1 package digests reproduced
   unchanged. One slice remains: in-compiler federation assembly, which owns
   the compiler's own structure scan -- `readSceneIr` is flat across both arms
   in the record (1.9 s on Digital Hub, 25.3 s on sixty5) because it reads the
   merged federation the adapter still writes. The ADR's gate 4 is the same predeclared bar after every slice: a
