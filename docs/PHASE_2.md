...
| Shape-preserving LOD | **Designed** ([ADR-0025](adr/0025-shape-preserving-lod-representation.md), Proposed; method comparison recorded, [#126](https://github.com/1n01raymond/naru/issues/126)) | Implement the `reduced` level under the `naru.progressive-package.1` bump and record ADR-0025 gates 1-3 |
...
| Shape-preserving LOD | Error/identity contract (**Proposed**, [ADR-0025](adr/0025-shape-preserving-lod-representation.md)) → compiler representation → scheduler admission → visual and interaction evidence | An ordering heuristic is not an LOD representation and cannot establish fidelity |
