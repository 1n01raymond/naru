...
| Shape-preserving LOD | **Representation implemented** ([ADR-0025](adr/0025-shape-preserving-lod-representation.md), Proposed; `--reduced-lod` emits a `reduced` level under `naru.progressive-package.1`, gate 1 recorded in [`artifacts/lod/reduced-level/`](../artifacts/lod/reduced-level/README.md), loader decodes it) | Studio selection by projected error with hysteresis, then the ADR-0025 gate-3 headed record |
...
| Shape-preserving LOD | Error/identity contract (**Proposed**, [ADR-0025](adr/0025-shape-preserving-lod-representation.md)) → compiler representation (**Implemented**, opt-in) → scheduler admission → visual and interaction evidence | An ordering heuristic is not an LOD representation and cannot establish fidelity |
...
a missing capability. Shape-preserving LOD and shared-cache authorization
remain implementation backlog. Workspace persistence
